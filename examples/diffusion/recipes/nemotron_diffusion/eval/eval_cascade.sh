...
# --- Checkpoint paths (Megatron Bridge format) ---
# These must be converted from HF format first using convert_checkpoints.py
CKPT_3B="${CKPT_3B:-/lustre/fsw/portfolios/coreai/users/snorouzi/megatron_exp/ministral_3b/iter_0012500}"
CKPT_8B="${CKPT_8B:-/lustre/fsw/portfolios/nvr/projects/nvr_lpr_llm/users/abhgarg/megatron_exp/ministral_8b_sbd64_llada_combine_before_weighting_1e_5_rerun_32/iter_0012500}"

# HF model IDs for config loading
HF_3B="${HF_3B:-/lustre/fsw/portfolios/nvr/users/snorouzi/models/Ministral-3-3B-Base-2512_converted}"
...
# --- Execution mode (passed through to eval_megatron.sh) ---
EXEC_MODE="${1:---parallel-tasks}"

# --- Common args ---
COMMON_ARGS="--eval-tasks gsm8k_cot,humaneval,mbpp,humaneval_plus,mbpp_plus --seeds 42 --modes dllm"
...
# For 32 steps: 8B does 16 steps, 3B does 16 steps
# For 16 steps: 8B does 8 steps, 3B does 8 steps
# For 1 step: skip cascade (only 1 step, no split possible)
# ================================================================
...
CASCADE_32_THR="${CKPT_8B}|16|${HF_8B}|${CKPT_3B}|16|${HF_3B}"
run_eval "cascade_8b_3b" "${CKPT_3B}" "${HF_3B}" "32" "0.9" "${CASCADE_32_THR}"
