...
class LLaDA2MoeRMSNorm(nn.Module):
    """RMSNorm layer equivalent to T5LayerNorm."""
...
class LLaDA2MoeMLP(nn.Module):
    """Gated MLP block for LLaDA2 MoE dense layers."""
...
class LLaDA2MoeGate(nn.Module):
    """Router gate with grouped top-k selection and sigmoid expert bias correction."""
...
class LLaDA2MoeDecoderLayer(nn.Module):
    """Single transformer decoder layer (attention + dense or MoE FFN)."""
...
class LLaDA2MoePreTrainedModel(PreTrainedModel):
    """Base class for LLaDA2 MoE models with weight initialization."""
...
class LLaDA2MoeModelLM(LLaDA2MoePreTrainedModel, GenerationMixin):
    """LLaDA2 MoE causal language model with lm_head."""
    ...
    def generate(
        ...
        block_diffusion_attention_mask = torch.where(block_diffusion_attention_mask, 0.0, float("-inf")).to(
            torch.bfloat16
        )
        ...
        num_transfer_tokens_schedule = self._get_num_transfer_tokens(block_length, denoising_steps_per_block)
        for num_block in range(prefill_blocks, num_blocks):
            ...
            if eos_id is not None and (x[0, prompt_length:current_window_end] == eos_id).any():
                break

        generated_answer = x[:, : prompt_length + gen_length]

        mask_positions = (generated_answer[0][input_ids.shape[1] :] == eos_id).nonzero(as_tuple=True)[0]
