...
#
# The 3B and 8B HF checkpoints need to be in MB format for eval_megatron.py.
# Run this once before running eval_cascade.sh.
#
# Usage:
...
MB_3B="/lustre/fsw/portfolios/coreai/users/snorouzi/megatron_exp/ministral_3b/iter_0012500"

if [ -d "${MB_3B}" ]; then
    echo "3B MB checkpoint already exists at ${MB_3B}, skipping."
else
    echo "Converting 3B HF → MB..."
    TRANSFORMERS_OFFLINE=1 HF_DATASETS_OFFLINE=1 python "${CONVERT_SCRIPT}" import \
        --hf-model "${HF_3B}" \
        --megatron-path "${MB_3B}" \
...
MB_8B="/lustre/fsw/portfolios/nvr/projects/nvr_lpr_llm/users/abhgarg/megatron_exp/ministral_8b_sbd64_llada_combine_before_weighting_1e_5_rerun_32/iter_0012500"
...
echo "You can now run eval_cascade.sh"
